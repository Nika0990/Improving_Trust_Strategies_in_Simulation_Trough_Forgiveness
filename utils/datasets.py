...
class OnlineSimulationDataSet(Dataset):
    ...
    class SimulatedUser:
        def __init__(self, user_improve, basic_nature, favorite_topic_method, **args):
            ...
            forgiveness_threshold = np.random.lognormal( mean=0.5, sigma=0.35)
            if history_window <= 1:
                forgiveness_count_max = 0
            else:
                forgiveness_count_max = np.random.randint(0, history_window - 1)
            ...
            self.ACTIONS = {0: ("correct - oracle", 0, user_strategies.correct_action),
                            ...
                            6: ("Forgiving Trustful LLM", basic_nature[5],
                                user_strategies.forgiving_trustful_with_llm(history_window, quality_threshold,
                                                                            forgiveness_threshold)),
                            7: ("Forgiving Trustful basic", basic_nature[6],
                                user_strategies.forgiving_trustful_with_max_threshold(history_window,
                                                                                      quality_threshold,
                                                                                      forgiveness_count_max)),
                            }
        ...
        for bot in bots:
            ...
            while (correct_answers < self.SIMULATION_TH) and not (
                ...
                for round_number in range(1, DATA_ROUNDS_PER_GAME + 1):  # start a new round
                    ...
                    previous_rounds += [(hotel, bot_message, user_action, review_id)]
